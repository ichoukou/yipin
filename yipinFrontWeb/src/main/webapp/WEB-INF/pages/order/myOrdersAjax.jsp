...
<%@include file="/WEB-INF/pages/include/taglibs.jsp"%>
	...
	<div class="rank_wrap cf">
		...
		<div class="o_tips">【合并付款】、【批量确认收货】<a href="javascript:;">如何操作？</a></div>
		<div class="rank">
			我的交易提醒：
	...
	<div class="data_show">
		...
		<div class="data_item">
			<div class="fold cf">
				<div class="col4">
					<c:if test="${isDisplayCheckBox}">	
						<span><input type="checkbox" class="checkbox" value="${order.orderId}"/></span>
					</c:if>
		...
		<div class="o_Ctrl">
			<c:if test="${isDisplayCheckBox}">	
				...
				<c:if test="${orderPage.params.status eq status[0]}">
					<a href="javascript:;" class="pan_btn J_bathpayment">合并付款已选择订单</a>
					<form id="J_bathpaymentform" action="" method="post" target="_blank"></form>
				</c:if>
				<c:if test="${orderPage.params.status eq status[2]}">
					<a href="javascript:;" class="pan_btn J_bathconfirmreceive">确认收货已选择订单</a>
				</c:if>
				</span>
			</c:if>
		</div>
