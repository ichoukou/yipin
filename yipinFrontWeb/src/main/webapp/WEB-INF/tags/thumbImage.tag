...
<c:if test="${not empty originalPath && not empty imageType}">
	<c:if test="${not empty imageType}">${_fileThumbPath}${originalPath}_${imageType}.${suffix}</c:if>	
</c:if>
<c:if test="${not empty originalPath && empty imageType}">
	<c:if test="${empty imageType}">${_filePath}${originalPath}</c:if>	
</c:if>