$(function(){
	...
	$.formValidator.initConfig({
 	    //validatorGroup: '2001', //多个表单使用窜号区分,单个表单无需使用此窜号
 	    formID: 'form1',
 	 	errorFocus:false,
 	    theme: 'Default'
 	});
...
//			}
	...
	$('#submitForm').bind('click',function(){
        ...
        //验证是否选择 城市品牌
        var selectValue=$("#selectCity").val();
        var divIsNull=$(".br-list").html();
        if(selectValue !='' && divIsNull !=''){
	        $(".br-list input:checkbox").each(function(){
	        	if(!this.checked){
	        		$('#brandTip').text('你至少选择1个可售品牌').css('color','red');
	        	}else{
	        		$('#brandTip').text('');
	        		isCheckbox=true;
	        	}
        ...
        var result = true;
        isCheckbox=false;
        var validatResult=result & isCheckbox;
		if(!validatResult){ //不通过验证，阻止
			alert(333);
			return false;
		}else{
//			$('#form1').submit();
