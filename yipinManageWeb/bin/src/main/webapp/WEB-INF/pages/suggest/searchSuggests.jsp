				...
				$.dialog({
			        ...
			        ok: function(){
				 		$.ajax({   
				 			   ...
				 			   success:function(data){   
				 					alert(data.info);
				 					window.location.href="${_ctxPath}/admin/suggest/suggest-searchSuggests.htm?suggestPage.params.type=1&suggestPage.params.status="+j
				 			   },
				 			   error:function(data){
				 				   //console.log(data);
				 			   }
			 ...
			 <!--end nav-->
				<div class="m-mt10p m-clear">
					<div class="body-nav subnav m-fl">
				        <ul>
				            <li><a id="suggestPage" <c:if test='${suggestPage.params.status == null || suggestPage.params.status == ""}'>class="current-chose"</c:if> href="${_ctxPath}/admin/suggest/suggest-searchSuggests.htm?suggestPage.params.type=1&suggestPage.params.status=">全部</a> </li>
				            <li><a id="suggestPage1" <c:if test="${suggestPage.params.status == 2 }">class="current-chose"</c:if> href="${_ctxPath}/admin/suggest/suggest-searchSuggests.htm?suggestPage.params.type=1&suggestPage.params.status=2">已读</a></li>
							<li><a id="suggestPage2" <c:if test="${suggestPage.params.status==1 }">class="current-chose"</c:if> href="${_ctxPath}/admin/suggest/suggest-searchSuggests.htm?suggestPage.params.type=1&suggestPage.params.status=1">未读</a></li>
						 	<li><a id="suggestPage3" <c:if test="${suggestPage.params.status==4 }">class="current-chose"</c:if> href="${_ctxPath}/admin/suggest/suggest-searchSuggests.htm?suggestPage.params.type=1&suggestPage.params.status=4">已回复</a></li>
						 	<li><a id="suggestPage4" <c:if test="${suggestPage.params.status==3 }">class="current-chose"</c:if> href="${_ctxPath}/admin/suggest/suggest-searchSuggests.htm?suggestPage.params.type=1&suggestPage.params.status=3">禁止</a></li>
				        </ul>
				    </div>
