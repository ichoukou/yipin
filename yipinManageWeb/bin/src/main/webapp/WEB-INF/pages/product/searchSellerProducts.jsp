...
<script type="text/javascript">
    ...
    <c:set var="STATUS_SELLY" value="<%= Product.STATUS_SELLY %>"></c:set>
										...
										<td width="8%">
											 <aut:authorize url="/seller/product/product-sellOut">
		                         				<c:if test="${product.status == STATUS_PASS 
		                         					&& (product.sellStatus == STATUS_SELL || product.sellStatus == STATUS_SELLY)  && product.inventory > 0}">
		                         					<p><a class="c-blue sellOut" href="javascript:;">售罄</a></p>
		                         				</c:if>
		                     				</aut:authorize>
											<aut:authorize url="/seller/product/product-delete">
												<c:choose>
													<c:when test="${product.status == STATUS_PASS && (product.sellStatus == STATUS_SELLY 
												|| product.sellStatus == STATUS_SELL) }"></c:when>
													<c:otherwise>
