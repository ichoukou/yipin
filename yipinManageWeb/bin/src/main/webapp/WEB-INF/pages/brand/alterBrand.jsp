		...
		<table border="0" class="add-table">
			<tr>
				...
				<td colspan="2">
					...
					英文名：<input type="text" class="addBrand-name" id="addBrand-en-name" name="brand.englishName" value="${brand.englishName}"  onblur="getBrandPinYin('addBrand-en-name')"/>
				</td>
				<td width="385"><span id="addBrandnameTip"></span></td>
			</tr>
			<tr>
				<td align="right" width="145"><span class="c-red">*</span>城市：</td>
				<td colspan="2">
				    <div class="m-fl">
		                <select name="brand.cityId" id="cityId">
		                   <option value="">选择城市</option>
	                           	<c:forEach items="${citys }" var="city">
									<option value="${city.cityId }"
									<c:if test="${brand.cityId==city.cityId }">
									selected = "selected"
									</c:if>
									>${city.name }</option>
								</c:forEach>
				...
				<td width="385"><span id="brandcityTip"></span></td>
			</tr>
			<tr>
				<td align="right" width="145">品牌集中营关联字符：</td>
				<td colspan="2">
					<input type="text" class="addBrand-name" id="brandNamePinYin" name="brand.firstChar" value="${brand.firstChar}"/>
				</td>
